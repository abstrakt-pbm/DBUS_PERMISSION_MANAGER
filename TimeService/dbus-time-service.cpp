...
DbusTimeService::DbusTimeService(std::string serviceName,
  ...
  permissionManagerProxy = std::unique_ptr<DbusPermissionManagerProxy>{
      new DbusPermissionManagerProxy("com.system.permissions",
...
DbusPermissionManagerProxy::DbusPermissionManagerProxy(
    std::string serviceName, std::string pathToObject) {
  ...
  dbusPrxy =
      sdbus::createProxy(std::move(permissionManagerProxy), std::move(objPath));

  std::cout << "dbus permission proxy initialized" << std::endl;
}

bool DbusPermissionManagerProxy::checkApplicationHasPermission(
    std::string pathToApplication, Permissions permission) {
...
uint64_t DbusTimeService::getSystemTime() {
  ...
  if (!permissionManagerProxy->checkApplicationHasPermission(
          dbusClientExecPath, Permissions::SystemTime)) {
