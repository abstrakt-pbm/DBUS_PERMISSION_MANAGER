...
std::string transferTimestamToReadableString(uint64_t timestamp) {
  std::chrono::system_clock::time_point representedTimePoint = std::chrono::system_clock::from_time_t(timestamp);
...
int main(int argc, char *argv[]) {
   ...
   while(true) {
      try {
         ...
         std::string readableTime = transferTimestamToReadableString(systemTimeStamp);
         std::cout << std::format("Current system time: {}", readableTime) << std::endl;

         break;
      } catch (const sdbus::Error &e) {
         if( e.getName() == "com.system.time.UnauthorizedAccess") {
            std::cout << "get unauthorized access, trying to get permission.." << std::endl;
            permissionManagerProxy.requestPermission(Permissions::SystemTime);
         } else {
            std::cout << std::format("get error while accessing dbus: Name: {} | Message: {}", e.getName().c_str(), e.getMessage()) << std::endl;
            break;
         }
      }
   }
